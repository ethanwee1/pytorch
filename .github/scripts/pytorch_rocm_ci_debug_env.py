...
Examples (a ``GITHUB_TOKEN``/``GH_TOKEN`` is recommended, and required in practice for the default
in-container mode's GitHub API calls; e.g. ``export GH_TOKEN=$(gh auth token)``):

    # Default: clone + install inside the container (no host clone, no bind mount).
    PYTHONPATH=scripts python3 scripts/pytorch_rocm_ci_debug_env.py <commit> trunk

    # Host-mapped: clone to a host temp dir and bind-mount it into the container.
    PYTHONPATH=scripts python3 scripts/pytorch_rocm_ci_debug_env.py <commit> trunk --host-mapped

    # Host-mapped reusing an existing clone (fetch/checkout the commit, then bind-mount it).
    PYTHONPATH=scripts python3 scripts/pytorch_rocm_ci_debug_env.py <commit> trunk \
        --repo-dir /path/to/pytorch

    # Resolve + print metadata, then exit (default mode: via the GitHub API, no clone).
    PYTHONPATH=scripts python3 scripts/pytorch_rocm_ci_debug_env.py <commit> trunk \
        --workflow-parse-only

    # Resolve + print metadata via a host clone, then exit (also prints clone-path).
    PYTHONPATH=scripts python3 scripts/pytorch_rocm_ci_debug_env.py <commit> trunk \
        --host-mapped --workflow-parse-only
"""
...
def filesystem_safe_commit_segment(ref: str) -> str:
    ...
    cleaned = "".join(c if (c.isalnum() or c in "._-") else "_" for c in ref.strip())
...
# ---------------------------------------------------------------------------
# ROCm/pytorch publish CLI
# ---------------------------------------------------------------------------


def artifact_url_exists(url: str) -> bool:
    request = urllib.request.Request(url, method="HEAD", headers={"User-Agent": "pytorch-rocm-ci-debug-env"})
    try:
...
def workflow_run_candidates(workflow_filename: str, *, branch: str, commit: str | None, token: str | None) -> list[tuple[str, int]]:
    encoded_wf = urllib.parse.quote(workflow_filename, safe="")
    if commit:
        head_sha = github_commit_full_sha(commit, token)
    ...
    url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/actions/workflows/{encoded_wf}/runs?{query}"
    payload = github_get_json_or_fail(url, token, what=f"runs for {workflow_filename}")
    runs = payload.get("workflow_runs") if isinstance(payload, dict) else None
    if not isinstance(runs, list) or not runs:
...
def resolve_publish_workflow(workflow: str, *, branch: str, commit: str | None, token: str | None) -> tuple[WorkflowResolution, int, str]:
    workflow_filename = workflow if workflow.endswith(".yml") else f"{workflow}.yml"
    last_error: Exception | None = None
    for head_sha, run_id in workflow_run_candidates(workflow_filename, branch=branch, commit=commit, token=token):
        try:
            resolved = resolve_workflow_via_github(head_sha, workflow_filename[:-4], token)
            url = artifact_zip_url(run_id, resolved.build_label)
            if not artifact_url_exists(url):
                print(f"artifact-missing: run={run_id} label={resolved.build_label}", file=sys.stderr)
                continue
            return resolved, run_id, url
...
def docker_run_publish_container(image_ref: str, head_sha: str, branch: str) -> str:
    command: list[str] = ["docker", "run", "-i", "-e", f"BRANCH={branch}", "-e", f"SHA1={head_sha}"]
    for key, val in DOCKER_RUN_ENV_LITERAL:
        command.extend(["-e", f"{key}={val}"])
    command.extend([
...
def print_resolution(resolved: WorkflowResolution, run_id: int, artifact_url: str) -> None:
    print(f"commit       = {resolved.head_sha}")
    print(f"run_id       = {run_id}")
    print(f"image_ref    = {ghcr_image_ref(resolved.image_stem, resolved.docker_directory_sha)}")
    print(f"build_label  = {resolved.build_label}")
    print(f"artifact_url = {artifact_url}")


def publish_prepared_container(container_id: str, resolved: WorkflowResolution, *, image: str, tag: str, push: bool) -> tuple[str, str]:
    ...
    run_checked([
        "docker", "commit",
        "--change", f"WORKDIR {CONTAINER_REPO_DIR}",
        "--change", f"LABEL pytorch.repo={REPO_OWNER}/{REPO_NAME}",
        "--change", f"LABEL pytorch.commit={resolved.head_sha}",
        "--change", f"LABEL pytorch.source_image={source_image}",
        container_id,
        primary,
    ])
...
def parse_arguments() -> argparse.Namespace:
    ...
    resolve = subparsers.add_parser("resolve", help="Resolve CI image/artifact metadata and exit.")
    resolve.add_argument("--workflow", default="trunk.yml")
    resolve.add_argument("--branch", default="main")
    resolve.add_argument("--commit", default="")
    publish = subparsers.add_parser("publish", help="Prepare, commit, and optionally push a PyTorch ROCm CI image.")
    publish.add_argument("--workflow", default="trunk.yml")
    publish.add_argument("--branch", default="main")
    publish.add_argument("--commit", default="")
    publish.add_argument("--image", required=True, help="Image without tag, e.g. docker.io/rocm/pytorch-nightly")
    publish.add_argument("--tag", default="nightly", help="Primary tag to publish")
    publish.add_argument("--push", action="store_true", help="Push primary and dated tags")
...
def main() -> int:
    ...
    try:
        resolved, run_id, artifact_url = resolve_publish_workflow(args.workflow, branch=args.branch, commit=args.commit.strip() or None, token=token)
        print_resolution(resolved, run_id, artifact_url)
        ...
        try:
            ...
            docker_exec_root_bash(container_id, publish_container_setup_script(resolved.head_sha, artifact_url))
            primary_tag, dated_tag = publish_prepared_container(container_id, resolved, image=args.image, tag=args.tag, push=args.push)
            write_github_output({
                "commit": resolved.head_sha,
                "short_sha": resolved.head_sha[:7],
                "run_id": str(run_id),
